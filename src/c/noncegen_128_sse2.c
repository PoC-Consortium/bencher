       ...
       for (uint64_t n = 0; n < local_nonces; n+=4) {
        ...
        for (size_t i = NONCE_SIZE - HASH_SIZE; i > (NONCE_SIZE - HASH_CAP); i -= HASH_SIZE) {
            ...
            if (i % 64 == 0) {
                ...
                mshabal_hash_fast_sse2(&local_128_fast, &cache[i * MSHABAL128_VECTOR_SIZE],
                                            &t1,
        ...
        for (size_t i = NONCE_SIZE - HASH_CAP; i > 0; i -= HASH_SIZE) {
            mshabal_hash_fast_sse2(&local_128_fast, &cache[i * MSHABAL128_VECTOR_SIZE], &t3,
                                        &cache[(i - HASH_SIZE) * MSHABAL128_VECTOR_SIZE],
                                        (HASH_CAP) >> 6);
        }

        // generate final hash
        mshabal_hash_fast_sse2(&local_128_fast, &cache[0], &t1, &final[0],
                                    (NONCE_SIZE + 16) >> 6);
...
void find_best_deadline_sse2(char *data, uint64_t scoop, uint64_t nonce_count, char *gensig,
  ...
  uint64_t mirrorscoop = 4095-scoop;
    ...
    for (uint64_t i = 0; i < nonce_count; i+=4) {
            ...
            mshabal_deadline_fast_sse2(&x, &gensig_simd, u1, u2, &term_simd, &d0, &d1, &d2, &d3);
