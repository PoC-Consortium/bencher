...
pub fn shabal256_deadline_fast(hash1: &[u8], hash2: &[u8], gensig: &[u8; 32]) -> u64 {
    let mut a = A_INIT;
    let mut b = B_INIT;
    let mut c = C_INIT;
    let mut w_high = 0u32;
    let mut w_low = 1u32;
    let hash1_ptr = hash1.as_ptr() as *const u32;
    let hash1 = unsafe { from_raw_parts(hash1_ptr, hash1.len() / 4) };
    let hash2_ptr = hash2.as_ptr() as *const u32;
    let hash2 = unsafe { from_raw_parts(hash2_ptr, hash2.len() / 4) };
    let gensig = unsafe { std::mem::transmute::<&[u8; 32], &[u32; 8]>(&gensig) };
    let mut term = [0u32; 8];
    term[0] = 0x80;

    input_block_add_dl(&mut b, &gensig[..], &hash1[..]);
    xor_w(&mut a, w_low, w_high);
    apply_p_dl(&mut a, &mut b, &c, gensig, &hash1[..]);
    input_block_sub_dl(&mut c, gensig, &hash1[..]);
    swap_bc(&mut b, &mut c);
    incr_w(&mut w_low, &mut w_high);

    input_block_add_dl(&mut b, &hash2[..], &term);
    xor_w(&mut a, w_low, w_high);
    apply_p_dl(&mut a, &mut b, &c, &hash2[..], &term);
    for _ in 0..3 {
        swap_bc(&mut b, &mut c);
        xor_w(&mut a, w_low, w_high);
        apply_p_dl(&mut a, &mut b, &c, &hash2[..], &term);
